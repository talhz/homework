...
class Passenger:
    def __init__(self,time,totalFloor):
        ...
        self.time_get_on = 0
        self.weight = random.normalvariate(65,10)
...
class Elevator:
    def __init__(self,totalFloor,timePerFloor,maxLoadingNum):
        self.upWaitingQueue = [SQueue() for i in range(totalFloor)]
        self.downWaitingQueue = [SQueue() for i in range(totalFloor)]
        self.getOffQueue = [SQueue() for i in range(totalFloor)]
        self.waitingPerson = 0
        self.loadingPerson = 0
        self.timeInTotal = 0#这个变量用来统计所有人在电梯上呆的总时间和，就是每个人待的时间相加
        self.currentFloor = 1
        self.currentDirection = 'up'
        self.remainingTime = timePerFloor
        self.timePerFloor = timePerFloor
        self.totalFloor = totalFloor
        self.waitingTimes = []
        self.totalNumPerson = 0
        self.maxLoadingNum = maxLoadingNum
    ...
    def onFloor(self):
        #判断是否在一个整数楼层
        return remainingTime == 0 or remainingTime == timePerFloor
    ...
    def shouldStop(self):
        ...
        if self.remainingTime:
            raise SyntaxError('stop at nonfloor')
    ...
    def arriveUp(self,currenttime):
        while (not self.upWaitingQueue[self.currentFloor-1].is_empty()
            ...
            self.getOffQueue[person.get_target_floor()-1].enqueue(person)
    ...
    def shouldChangeDirection(self):
        if self.currentDirection == 'up':
            if self.currentFloor == self.totalFloor:
                return True
